  ...
  <div class="table-container">
    ...
    <div class="search-bar">
      <div class="search-group">
        <el-input
          v-model="searchId"
          placeholder="供应商ID精确查询（如S001）"
          class="search-input"
        />
        <el-input v-model="searchName" placeholder="供应商名称模糊查询" class="search-input" />
      </div>
      <div class="button-group">
        ...
        <el-button type="success" class="action-btn" @click="handleAdd">新增供应商</el-button>
      </div>
    </div>

    <!-- 表格区域 -->
    <el-table :data="paginatedData" class="modern-table" style="width: 100%">
      <el-table-column label="供应商ID" prop="supplier_id" align="center" />
      <el-table-column label="供应商名称" prop="supplier_name" align="center" />
...
const originalTableData = [
  ...
  {
    supplier_id: 'S003',
    supplier_name: '广州物流配送中心',
    contact_person: '王五',
    contact_phone: '13700137000',
    address: '广州市天河区珠江新城100号',
  },
  {
    supplier_id: 'S004',
    supplier_name: '深圳电子元件供应商',
    contact_person: '赵六',
    contact_phone: '13600136000',
    address: '深圳市南山区科技园55号',
  },
  {
    supplier_id: 'S005',
    supplier_name: '杭州服装贸易公司',
    contact_person: '孙七',
    contact_phone: '13500135000',
    address: '杭州市西湖区文三路30号',
  },
]
...
const fetchData = () => {
  ...
  // ID精确查询
  if (searchId.value) {
    data = data.filter((item) => item.supplier_id === searchId.value)
  }

  // 名称模糊查询
  if (searchName.value) {
    const keyword = searchName.value.toLowerCase()
    data = data.filter((item) => item.supplier_name.toLowerCase().includes(keyword))
  }
...
const handleEdit = (index, row) => {
  alert(`编辑索引: ${index}, 供应商ID: ${row.supplier_id}`)
}
const handleDelete = (index, row) => {
  alert(`删除索引: ${index}, 供应商ID: ${row.supplier_id}`)
}
</script>
